...
function trash(items) {
  ...
  notify(lang['message-delete'], function () {
    performLfmRequest('delete', {
      ...
      is_file: items.map(function (item) { return item.is_file; }),
      id: items.map(function (item) { return item.id; }),
    }).done(refreshFoldersAndItems)
  });
}
...
function download(items) {
  ...
  location.href = lfm_route + '/download?download_id=' + id;
}
...
function dialog(title, value, callback, option) {
  $('#dialog').find('input').val(value);
  $('#dialog').find('#rename_tags').val(option);
