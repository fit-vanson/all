...
//        dd($data_arr,json_decode($a,true));
    ...
    public function categoriesArray($data){
        $jsonObj= [];
        foreach ($data as $item){
            $data_arr['id'] = $item['id'];
            $data_arr['title'] = $item['category_name'];
            $data_arr['image'] = $item['image'];
    ...
    private  function getWallpaper($data){
        ...
        foreach ($data['data'] as $item){
            ...
            $data_arr['review'] =  false;
            $data_arr['premium'] =  false;
            $data_arr['color'] = '232c47';
            $data_arr['size'] = "424.52 KB";
            $data_arr['resolution'] = "1080 X 2340";

            $data_arr['comment']= true;

            $data_arr['comments'] = 0;
            $data_arr['downloads'] = $item['like_count'];

            $data_arr['views'] = $item['view_count'];
            $data_arr['shares'] = $item['view_count'];
            $data_arr['sets'] = $item['view_count'];
            $data_arr['trusted'] = false;


            $data_arr['userid'] = 11;
            $data_arr['userimage'] = "https://lh3.googleusercontent.com/a-/AOh14GjpZD7oDCjepeJahIo6pC-tM9zUl7-uDNo1pYDpfg";
            $data_arr['type'] =  "image/jpeg";
            $data_arr['extension'] = "jpeg";
...
//                $data_arr['category']['title'] = $value->name ;
